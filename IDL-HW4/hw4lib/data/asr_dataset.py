...
class ASRDataset(Dataset):
    def __init__(
        ...
        self.fbank_files = sorted([fname for fname in os.listdir(self.fbank_dir) if fname.endswith('.npy')])
        ...
        if self.partition != "test-clean":
            ...
            self.text_files = sorted([fname for fname in os.listdir(self.text_dir) if fname.endswith(".npy")])
    ...
    def collate_fn(self, batch) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        feat_lengths = torch.LongTensor([item[0].shape[1] for item in batch]) # B
